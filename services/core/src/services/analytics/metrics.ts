 ...
 * deterministic (seeded via `Rng`). No framework, no DB.
 *
 * What is NOT here, and deliberately so: loyalty tiers and points. The original
 * version of this file built a `Bronze/Silver/Gold/Platinum` distribution and a
 * "points issued" figure. Loyalty was deleted from the product in the rebuild,
 * so those panels would not have been placeholder numbers for a real feature —
 * they would have been a whole feature invented on a dashboard. `newVsReturning`
 * carries the audience-composition story instead, and it maps onto attendance
 * data that actually exists.
 */

import type { AnalyticsKpis, AnalyticsRange, SeriesPoint, StatTile } from './types';
...
export function buildStats(rng: Rng, scope: string): StatTile[] {
  ...
  const people = Math.round(rng.int(640, 1480) * (scope === 'all' ? 1 : 0.3));
  const repeat = Math.round(rng.int(120, 460) * scale);
  ...
  return [
    ...
    {
      id: 'people',
      label: scope === 'all' ? 'People on your roll' : 'Registered',
      value: people.toLocaleString(),
      change: sign(rng.int(-3, 15)),
      trend: rng.chance(0.75) ? 'up' : 'down',
    },
    {
      // Replaces the old "Active streaks" tile — streaks were a loyalty concept.
      id: 'repeat',
