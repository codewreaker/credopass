  ...
  useEffect(() => {
    if (eventId && events.length > 0 && !sessionInitialized) {
      ...
      * Subscribe for updates from some external system, calling setState in a callback function when external state changes.
      ...
      /Users/israelagyeman-prempeh/Dev-Ops/credopass/apps/web/src/Pages/CheckIn/index.tsx:106:7
        104 |   useEffect(() => {
        105 |     if (eventId && events.length > 0 && !sessionInitialized) {
      > 106 |       initializeEventSession(eventId);
            |       ^^^^^^^^^^^^^^^^^^^^^^ Avoid calling setState() directly within an effect
        107 |     }
        108 |   }, [eventId, events, sessionInitialized, initializeEventSession]);
        109 |
       */
